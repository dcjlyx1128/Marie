...
def organize(
    ...
    to: Optional[Path] = typer.Option(None, "--to", file_okay=False, help="归档根目录(默认整理进原文件夹;可设为别的目录)"),
    ...
    actions = plan(files, base, lambda f: decisions[f.path])
    table = Table(title=f"整理预览:{folder} → {base}  (共 {len(actions)} 个文件)")
    ...
    for a in actions:
        table.add_row(a.src.name, a.category, str(a.dest.relative_to(base)))
    console.print(table)
...
@app.command()
def watch(
    folder: Path = typer.Argument(..., exists=True, file_okay=False, help="要监听的文件夹"),
    to: Optional[Path] = typer.Option(None, "--to", file_okay=False, help="归档根目录(默认归到被监听文件夹内)"),
    config_path: Optional[Path] = typer.Option(None, "--config", help="指定配置文件路径"),
):
    """监听文件夹,新文件到达后自动归类(只移不删,可 marie undo 撤销)。Ctrl-C 停止。"""
    from .config import load_config
    from .pipeline import archive_root
    from . import watcher

    config = load_config(folder, config_path)
    if to:
        config.base = str(to)
    console.print(f"[green]👀 正在监听 [bold]{folder}[/] → [bold]{archive_root(folder, config)}[/](Ctrl-C 停止)…[/]")
    watcher.watch(folder, config, console.print)
